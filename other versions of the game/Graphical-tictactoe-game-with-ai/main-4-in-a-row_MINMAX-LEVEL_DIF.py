...
class AI:  
    def __init__(self, level=1, player=2):  
        self.level = level  # Level ranges from 0 (easy) to 5 (very difficult)  
        self.player = player  
    ...
    def choose_move(self, board):  
        depth = [1, 2, 3, 4, 5, 6][self.level]  # Different depths for different difficulties  
        if self.level == 0:  
            return self.random_move(board)  
        else:  
            _, move = self.minimax(board, depth, float('-inf'), float('inf'), True)  
            return move  
...
def main():  
    pygame.init()  
    game = Game()  
    ai = AI(level=5, player=2)  # Level 3 for 'medium'  5 'extremely' difficulty, player 2 might be 'O'  
